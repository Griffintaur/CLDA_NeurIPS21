...
def protoloss(src_feat,target_feat,label,mask=None,normalize_feature=True):
    if normalize:
        src_feat = normalize(src_feat, axis=-1)
        proto = normalize(target_feat, axis=-1)
    logits = -((src_feat - proto)**2)
    # breakpoint()
    if mask is not None:
        loss = (F.cross_entropy(logits, label,reduction='none')*mask).mean()
    else:
